...
impl GoldbergGenerator {
    ...
    /// Set the target language for the emulator
    pub fn set_language(&self, game_path: &Path, language: &str) -> Result<(), String> {
        let settings_dir = game_path.join("steam_settings");
        if !settings_dir.exists() {
            fs::create_dir_all(&settings_dir).map_err(|e| e.to_string())?;
        }

        let lang_file = settings_dir.join("force_language.txt");
        fs::write(&lang_file, language).map_err(|e| format!("Failed to set language: {}", e))?;
        
        // Also create language.txt as fallback
        let lang_file_fallback = settings_dir.join("language.txt");
        let _ = fs::write(&lang_file_fallback, language);

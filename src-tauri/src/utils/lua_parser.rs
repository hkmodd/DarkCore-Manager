...
pub fn parse_content(content: &str) -> Result<ScriptData, Box<dyn Error>> {
    ...
    // Regex compilation - UPDATED FOR PARITY
    // 3-arg: addappid(ID, 1, "KEY") - supports " and '
    let re_addappid_with_key_3arg =
        Regex::new(r#"addappid\s*\(\s*(\d+)\s*,\s*\d+\s*,\s*["']([0-9a-fA-F]+)["']\s*\)"#)?;

    // 2-arg: addappid(ID, "KEY") - supports " and '
    let re_addappid_with_key_2arg =
        Regex::new(r#"addappid\s*\(\s*(\d+)\s*,\s*["']([0-9a-fA-F]+)["']\s*\)"#)?;

    // 1-arg: addappid(ID)
    let re_addappid_only = Regex::new(r#"addappid\s*\(\s*(\d+)\s*\)"#)?;
    ...
    for line in &lines {
        ...
        if let Some(cap) = re_addappid_with_key_3arg.captures(trimmed) {
            if let (Some(id_match), Some(key_match)) = (cap.get(1), cap.get(2)) {
                ...
                // CRITICAL: DLC section items with keys are ALSO DLCs!
                if current_section == DepotCategory::DlcDepot
                    && !data.dlcs.iter().any(|d| d.app_id == id)
                    && Some(id) != main_app_id
                {
                    ...
                    if !dlc_name.to_lowercase().contains("depot") {
        ...
        } else if let Some(cap) = re_addappid_with_key_2arg.captures(trimmed) {
            if let (Some(id_match), Some(key_match)) = (cap.get(1), cap.get(2)) {
                let id = id_match.as_str().parse::<u32>()?;
                let key = key_match.as_str().to_string();

                if !temp_keys.contains_key(&id) {
                    temp_keys.insert(id, (key.clone(), comment_name.clone()));
                    temp_depot_cats.insert(id, current_section.clone());
                    data.depot_keys.insert(id, key);
                }

...
impl RawSteamInfo {
    ...
    fn extract_depots(&self, target_lang: &str) -> Vec<DepotNode> {
        ...
        if let Some(depots) = self.data.get("data").and_then(|d| d.get(&self.appid)).and_then(|a| a.get("depots")).and_then(|d| d.as_object()) {
            for (key, val) in depots {
                if let Ok(_) = key.parse::<u64>() {
                    ...
                    // Filter Logic:
                    // 1. If No Lang (Common) -> Keep
                    // 2. If Target Lang -> Keep
                    // 3. Else -> Skip
                    ...
                    if is_common || is_target {
                        ...
                        if let Some(manifests) = val.get("manifests").and_then(|m| m.get("public")) {
                             if let Some(g) = manifests.get("gid").and_then(|v| v.as_str()) {
                                 gid = Some(g.to_string());
                             }
                        }
                        
                         if let Some(s) = val.get("maxsize").and_then(|v| {
                             if let Some(s_val) = v.as_str() {
                                 Some(s_val.to_string())
                             } else if let Some(f_val) = v.as_f64() {
                                 Some(f_val.to_string())
                             } else {
                                 None
                             }
                         }) { // usually string in JSON
                             // API sometimes returns number, sometimes string?
                             // Actually maxsize is usually a string in steamcmd api output
